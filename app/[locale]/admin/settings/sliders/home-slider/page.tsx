...
import {
  createSliderDispatch,
  deleteSliderDispatch,
  getAllSliderImageDispatch,
...
export default function HomeSliderPage() {
    ...
    const resizeImage = (file) => {
      return new Promise((resolve) => {
        Resizer.imageFileResizer(
          ...
          "blob" // ✅ Çıktıyı doğrudan File olarak al
        );
      });
    };
  ...
  return (
    <div className="p-6">
      <div
        className={
          "flex items-center flex-wrap border rounded p-6 flex-row gap-x-6 gap-y-6"
        }
      >
        {sliders.map((slider, index) => (
          <div key={index} className="flex flex-col items-center">
            ...
            >
              <MdCancel
                className={"text-red-600 absolute z-10 right-0 top-0"}
                onClick={() => {
              ...
              />
              <Image
                src={`${process.env.NEXT_PUBLIC_RESOURCE_API}${slider.url}`}
                alt="Preview"
                width={192} // 48 * 4
                height={96} // 24 * 4
        ...
        {selectedImages.map((slider, index) => (
          <div key={index} className="flex flex-col items-center">
            ...
            >
              <MdCancel
                className={"text-red-600 absolute right-0 top-0"}
                onClick={() => {
                  const newImageState = selectedImages.filter(
                    (_, i) => i !== index
                  );

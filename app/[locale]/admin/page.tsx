...
function AdminPage() {
    ...
    const rowExpansionTemplate = (data) => {
        return (
            ...
            >
                <Column field={"image"} header={'Image'} body={(row)=> (
                    <Image src={`${process.env.NEXT_PUBLIC_RESOURCE_API}${row.image}`} alt={row.image} width={35}
                           height={35}
                           className={"rounded hover:w-44 hover:h-44 transition-all cursor-zoom-in"} />
                )}/>
            ...
            >
              ...
              {data.shippingAddress.address} - {data.shippingAddress.street} -{" "}
                ...
                <Column header={'Payment Status'} body={(row) => (
                    <Dropdown value={row.status} className={'text-xs'} options={['PAID', 'SHIPPED', 'CANCELLED']}
                              onChange={(e) => {

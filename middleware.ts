...
export default async function middleware(req) {
  ...
  const token = req.cookies.get('next-auth.session-token')?.value || req.cookies.get('__Secure-next-auth.session-token');
...
export const config = {
  matcher: ['/', '/(tr|en)/:path*', `/(tr|en)/admin/:path*`, `/(tr|en)/profile/:path*`],
};
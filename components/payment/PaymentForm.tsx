...
import {MdArrowLeft, MdArrowRight, MdNextPlan, MdSkipNext} from "react-icons/md";
                ...
                >
                            ...
                            {errors.billingAddress.address}
                                                ...
                                                <div className={'flex w-full justify-end'}>
                                                    <Button size={'small'} onClick={() => {
                                                        if (values.buyer.name === "" || errors.buyer?.name || errors.buyer?.surname || errors.buyer?.email
                                                            || errors.buyer?.gsmNumber || errors.shippingAddress?.city
                                                            || errors.shippingAddress?.state
                                                            || errors.shippingAddress?.street
                                                            || errors.shippingAddress?.address) {
                                                            toast.info('Lütfen eksik alanları doldurunuz')
                                                        } else {
                                                            setStep(1)
                                                            window.scrollTo({
                                                                top: 0,
                                                            ...
                                                            icon={<MdArrowRight size={32}/>}>Ödeme Bilgileri</Button>
                                                </div>
                                            </div>
                                        </>
                                    )}

                                {
                                    step === 1 && (
                                        <>
                                            <div className={`flex flex-col relative gap-y-6 rounded-lg px-2 py-8`}>
                                                <div className="p-inputgroup flex-1 relative ">
                                                    ...
                                                    <InputText
                                                        ...
                                                        className={`border  px-4 placeholder:text-md  ${
                                                            ...
                                                            touched.paymentCard?.cardHolderName && "border-red-600"
                                                        }`}
                                                        placeholder={t(
                    ...
                    <span className="p-inputgroup-addon">
                      <BsCreditCard2Front size={24}/>
                                                ...
                                                <div
                                                    className="grid grid-cols-1 md:grid-cols-2 gap-2 justify-between mb-4">
                                            ...
                                            <div className={'flex flex-row w-full justify-between px-4'}>
                                                <Button onClick={() => {
                                                    ...
                                                    window.scrollTo({
                                                        ...
                                                        className={'!p-1 !px-4'}
                                                        icon={<MdArrowLeft size={32}/>}>Geri</Button>
                                                <Button onClick={() => {
                                                    setStep(2)
                                                    window.scrollTo({
                                                        top: 0,
                                                        ...
                                                        className={'!p-1 !px-4'} icon={<MdArrowRight size={32}/>}>Alışveriş
                                                    Detayı</Button>
                                            </div>
                                        </>
                                    )
                                }
                                {
                                    step === 2 && (
                                        <div className={'flex flex-col mx-4 gap-y-6'}>
                                            <div
                                                className={'flex md:flex-row justify-between flex-col sm:gap-y-4 gap-y-4 gap-x-4'}>
                                                    <span className={'bg-gray-100 p-4 rounded-lg'}>
                                                        <p className={'font-bold'}>Adres:</p>
                                                        <p>{values.shippingAddress.address}</p>
                                                    </span>
                                                <span className={'bg-gray-100 p-4 rounded-lg'}>
                                                        <p className={'font-bold'}>Fatura Adresi:</p>
                                                        <p>{values.billingAddress.address ? values.billingAddress.address : values.shippingAddress.address}</p>
                                                    </span>
                                            </div>
                                            <div className={'flex flex-col gap-y-3 bg-gray-100 p-4 rounded-lg'}>
                                                <p className={'font-bold'}>Müşteri Bilgileri:</p>
                                                <span className={'flex flex-row gap-x-2'}><p
                                                    className={'font-semibold'}>Ad: </p>{values.buyer.name}</span>
                                                <span className={'flex flex-row gap-x-2'}><p
                                                    className={'font-semibold'}>Soyad: </p>{values.buyer.surname}</span>
                                                <span className={'flex flex-row gap-x-2'}><p
                                                    className={'font-semibold'}>E-mail: </p>{values.buyer.email}</span>
                                                <span className={'flex flex-row gap-x-2'}><p
                                                    className={'font-semibold'}>Telefon: </p>{values.buyer.gsmNumber}</span>
                                            </div>
                                            <Button onClick={() => {
                                                setStep(0)
                                                window.scrollTo({
                                            ...
                                            }} disabled={loading} className={'flex justify-center'}>Bilgileri Düzenle</Button>
                                            ...
                                            <div className={'flex flex-col gap-y-4'}>
                                                {
                                                    errors.paymentCard?.cardNumber &&
                                                    <small
                                                        className={'text-sm text-red-600'}>*{errors.paymentCard.cardNumber}</small> ||
                                                    errors.paymentCard?.cardHolderName &&
                                                    <small
                                                        className={'text-sm text-red-600'}>*{errors.paymentCard.cardHolderName}</small> ||
                                                    errors.paymentCard?.expireMonth && <small
                                                        className={'text-sm text-red-600'}>*{errors.paymentCard.expireMonth}</small> ||
                                                    errors.paymentCard?.expireYear && <small
                                                        className={'text-sm text-red-600'}>*{errors.paymentCard.expireYear}</small> ||
                                                    errors.buyer?.name && <small
                                                        className={'text-sm text-red-600'}>*{errors.buyer.name}</small> ||
                                                    errors.buyer?.surname && <small
                                                        className={'text-sm text-red-600'}>*{errors.buyer.surname}</small> ||
                                                    errors.buyer?.email && <small
                                                        className={'text-sm text-red-600'}>*{errors.buyer.email}</small> ||
                                                    errors.buyer?.gsmNumber && <small
                                                        className={'text-sm text-red-600'}>*{errors.buyer.gsmNumber}</small> ||
                                                    errors.shippingAddress?.address && <small
                                                        className={'text-sm text-red-600'}>*{errors.shippingAddress.address}</small> ||
                                                    errors.shippingAddress?.city && <small
                                                        className={'text-sm text-red-600'}>*{errors.shippingAddress.city}</small> ||
                                                    errors.shippingAddress?.state && <small
                                                        className={'text-sm text-red-600'}>*{errors.shippingAddress.state}</small> ||
                                                    errors.shippingAddress?.street && <small
                                                        className={'text-sm text-red-600'}>*{errors.shippingAddress.street}</small> ||
                                                    errors.billingAddress?.address && <small
                                                        className={'text-sm text-red-600'}>*{errors.billingAddress.address}</small> ||
                                                    errors.billingAddress?.city && <small
                                                        className={'text-sm text-red-600'}>*{errors.billingAddress.city}</small> ||
                                                    errors.billingAddress?.state && <small
                                                        className={'text-sm text-red-600'}>*{errors.shippingAddress.state}</small> ||
                                                    errors.billingAddress?.street && <small
                                                        className={'text-sm text-red-600'}>*{errors.shippingAddress.street}</small>
                                                }

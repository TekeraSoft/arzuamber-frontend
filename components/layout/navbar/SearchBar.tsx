...
function SearchBar({ SearchOpen, setSearchOpen }) {
  ...
  const {searchProducts, loading} = useSelector((state:RootState)=> state.search)
  ...
  return (
    <div className="w-full flex justify-start lg:justify-end items-center">
      <motion.div
          ...
          className="hidden lg:flex relative flex-col justify-between items-center bg-mywhite text-black px-5 py-1 border-b md:border-none min-w-full gap-1"
      >
        <div
        ...
        <div className="flex w-full transition duration-300 hover:shadow hover:shadow-secondary rounded-lg">
          <span className="p-input-icon-right w-full">
            {loading ? <SpinnerIcon />: (searchProducts.length > 0 ? <MdCancel onClick={()=> {
              dispatch(clearState())
              setSearchTerm("")
            }} size={18} className={'cursor-pointer'} />:<BiSearch size={18} />) }
            <InputText onFocus={() => setIsFocused(true)} onChange={(e)=> setSearchTerm(e.target.value)} className={'w-full rounded !shadow-none !outline-none'} placeholder={t('SearchBar.placeHolder')}/>
          </span>
        </div>
        {
            isFocused && searchProducts.length > 0 && (
                <div
                    ...
                    className={'bg-white border max-h-[345px] w-full flex flex-col gap-y-3 overflow-y-auto shadow-md rounded-lg top-12 p-3 absolute z-40 animate__animated animate__fadeIn'}>
                  {
                    searchProducts.map((item,index) => (
                        <button key={index} onClick={()=> {
                          ...
                          <Image src={`${process.env.NEXT_PUBLIC_RESOURCE_API}${item.colorSize[0].images[0]}`}
                                 width={40} height={40}
                          ...
                          <div className={'flex flex-row items-center justify-between w-full'}>
                            ...
                            <strong>{item.price.toLocaleString("tr-TR", {
                              style: "currency",
                              currency: "TRY",
                            })}</strong>
                          </div>
                              ...
                              item.colorSize.flatMap((cs,index) => (
                                  <span key={index} className={'flex flex-row gap-x-2'}>
                                    ...
                                    cs.stockSize.map((ss,index) => (
                                        <span key={index} className={`${ss.stock === 0 ? 'line-through':''} border px-2 h-6 flex items-center justify-center rounded text-[10px]`}>{ss.size}</span>
                                    ))
                                  }
                                  </span>
                              ))
      ...
      {SearchOpen && (
          <motion.div
              ...
              } relative bg-mywhite justify-center text-black px-5 py-1 border-b md:border-none min-w-full gap-1`}
          >
            ...
            {loading ? <SpinnerIcon/> : (searchProducts.length > 0 ? <MdCancel onClick={() => {
              ...
              <InputText onFocus={() => setIsFocused(true)} onChange={(e) => setSearchTerm(e.target.value)}
                         className={'w-full rounded !shadow-none !outline-none'}
            ...
            </div>
            {
                isFocused && searchProducts.length > 0 && (
                    <div
                        ref={searchResultsRef}
                        className={'bg-white border max-h-[345px] flex flex-col gap-y-3 overflow-y-auto rounded-lg top-12 p-3 absolute z-40 animate__animated animate__fadeIn'}>
                      {
                        searchProducts.map((item, index) => (
                            <button key={index} onClick={() => {
                              router.push(`/product/${item.slug}`)
